...
export default function LoginInput({ placeholder, bottom, ...props }) {
  ...
  return (
    ...
    {meta.touched && meta.error && !bottom && (
      ...
      <input className={meta.touched && meta.error ? 'input_error_border': ''} placeholder={placeholder} {...field} {...props} type={field.type} name={field.name} />
      {meta.touched && meta.error && bottom && (
      ...
      { meta.touched && meta.error && (
        <i className='error_ico' style={{top: `${!bottom && !desktopView ? '63%' : '15px'}` }}></i>
      )
       } 
    </div>
  )
